...
const createContent = [
  ...
  body('type').notEmpty().withMessage('Content type is required').isIn(['video', 'article', 'quiz', 'image', 'audio', 'resource']).withMessage('Invalid content type'),
  ...
  body('order').optional().isInt({ min: 0 }).withMessage('Order must be non-negative'),
...
const createContentBody = [
  body('chapter').notEmpty().withMessage('Chapter ID is required').isMongoId().withMessage('Invalid chapter ID'),
  body('type').optional().isIn(['video', 'article', 'quiz', 'image', 'audio', 'resource']).withMessage('Invalid content type'),
  body('title').trim().notEmpty().withMessage('Title is required').isLength({ max: 300 }).withMessage('Title too long'),
  body('description').optional().trim().isLength({ max: 10000 }).withMessage('Description too long'),
  body('thumbnail').optional().trim().isURL().withMessage('Thumbnail must be a valid URL'),
  body('order').optional().isInt({ min: 0 }).withMessage('Order must be non-negative'),
