...
/**
 * Displays in-progress courses with progress bars and resume actions.
 */
const CurrentCourseTable = ({ courses, onResume }) => {
  return (
    <div className="overflow-hidden rounded-[32px] border border-gray-200 bg-white/95 shadow-sm backdrop-blur-sm transition-colors duration-300 dark:border-white/[0.12] dark:bg-white/[0.06] dark:shadow-[0_8px_32px_rgba(0,0,0,0.45)] dark:backdrop-blur-xl">
      <table className="w-full text-left">
        ...
        <tbody className="divide-y divide-gray-200 transition-colors duration-300 dark:divide-white/[0.06]">
          {courses.length === 0 ? (
            <tr>
              <td colSpan={3} className="px-6 py-10 text-center text-sm text-gray-400 dark:text-white/40">
            ...
            courses.map((course) => (
              <tr
                key={course.courseId || course.id || course.title}
              ...
              >
                ...
                <td className="px-6 py-4 text-right">
                  ...
                  >
                    {course.completed || course.progress >= 100 ? 'Review' : 'Resume'}
                  </Button>
                </td>
              </tr>
            ))
