...
const LearnerDashboardContent = () => {
  ...
  const stats = useMemo(() => {
    const hoursLearnedRaw =
      (enrolledCourses || []).reduce((sum, c) => {
        const v =
          Number(c.hoursLearned ?? c.hours ?? c.timeSpentHours ?? c.timeSpent ?? 0) || 0;
        return sum + v;
      }, 0) || 0;
    const hoursLearnedRounded = Math.round(hoursLearnedRaw * 10) / 10;
    const hoursLearnedDisplay =
      hoursLearnedRaw > 0 && hoursLearnedRounded === 0
        ? '<0.1'
        : hoursLearnedRounded % 1 === 0
          ? String(hoursLearnedRounded)
          : hoursLearnedRounded.toFixed(1);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {loading
          ? [1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="rounded-[24px] border border-gray-200 bg-white/95 p-6 shadow-sm backdrop-blur-sm dark:border-white/[0.14] dark:bg-white/[0.07]"
              >
