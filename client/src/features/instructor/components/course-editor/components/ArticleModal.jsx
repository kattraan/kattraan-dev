...
const ArticleModal = ({ isOpen, onClose, onSave, isSaving, existingContent }) => {
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-[#1E1E1E] border border-white/10 rounded-[28px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between p-5 border-b border-white/5 bg-white/[0.01]">
                    <h2 className="text-[17px] font-bold text-white tracking-tight mx-auto">
                        {existingContent ? 'Edit Article' : 'Add Article'}
                    </h2>
                ...
                <div className="p-6 space-y-5">
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 ml-1">Article Title</label>
                        <input 
                            type="text" 
                            value={title}
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 ml-1">Article Body</label>
                        <textarea 
                            value={body}
                            onChange={(e) => setBody(e.target.value)}
                            placeholder="Type your article content here..."
                            rows={12}
                            className="w-full bg-white/5 border border-white/5 rounded-2xl px-4 py-3 text-[13px] font-medium text-white/80 focus:outline-none focus:border-primary-pink/30 transition-all resize-none scrollbar-thin scrollbar-thumb-white/10"
                        />
                    </div>
                    ...
                    >
                        ...
                        {isSaving ? 'Saving...' : existingContent ? 'Update Article' : 'Add Article'}
                    </button>
                </div>
            </div>
        </div>
