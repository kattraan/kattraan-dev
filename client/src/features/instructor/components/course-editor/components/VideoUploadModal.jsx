...
import { X, Upload, Video, Loader2, AlertCircle, Trash2, Edit2, Plus } from 'lucide-react';
 ...
 */
const VideoUploadModal = ({ isOpen, onClose, onSave, isSaving, existingContent }) => {
    ...
    const handleAddSubtitle = () => {
        setSubtitles([...subtitles, { id: Date.now(), language: 'English', file: null }]);
    };
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-[520px] bg-[#1E1E1E] border border-white/10 rounded-[28px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh] scrollbar-thin scrollbar-thumb-white/10">
                    ...
                    <div 
                        onClick={() => !hasVideoSelected && !hasExistingVideo && videoInputRef.current?.click()}
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    {thumbnailPreview ? (
                                        <div className="w-14 h-14 rounded-xl overflow-hidden border border-white/10 shrink-0">
                                            <img src={thumbnailPreview} alt="Thumbnail" className="w-full h-full object-cover" loading="lazy" />
                                        </div>
                                    ) : (
                                    ...
                                    <div className="overflow-hidden">
                                        <p className="text-[14px] font-bold text-white truncate max-w-[280px]">
                                            {videoFile?.name || existingContent?.metadata?.fileName || existingContent?.title || 'Video'}
                                        </p>
                                        <p className="text-[11px] font-black uppercase tracking-wider text-white/30 mt-0.5">
                                            MP4 • {videoFile 
                                                ? (videoFile.size / (1024 * 1024)).toFixed(2) + ' MB'
                                                : existingContent?.metadata?.fileSize 
                                                    ? (existingContent.metadata.fileSize / (1024 * 1024)).toFixed(2) + ' MB'
                                                    : 'Video'
                                            }
                                        </p>
                                    </div>
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 ml-1">Video Title</label>
                        <input 
                            type="text" 
                            value={title}
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-[20px]">
                        <div className="space-y-1 pr-4">
                            <h4 className="text-[13px] font-bold text-white">Add Thumbnail (Optional)</h4>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                            {thumbnailPreview && (
                                <div className="w-12 h-12 rounded-lg overflow-hidden border border-white/10 shadow-lg">
                                    <img src={thumbnailPreview} alt="Preview" className="w-full h-full object-cover" loading="lazy" />
                                </div>
                            )}
                            <button 
                            ...
                            >
                                {thumbnailFile || thumbnailPreview ? 'Change Thumbnail' : 'Upload Thumbnail'}
                            </button>
                        </div>
                    ...
                    <div className="space-y-4">
                        {subtitles.length > 0 && (
                            <div className="grid grid-cols-[1fr_2fr_auto] gap-4 px-1">
                                <label className="text-[10px] font-black uppercase tracking-[0.15em] text-white/20">Language</label>
                                <label className="text-[10px] font-black uppercase tracking-[0.15em] text-white/20">Upload Subtitle (vtt only)</label>
                                <div className="w-6" />
                            </div>
                        )}
                        
                        {subtitles.map((sub) => (
                            <div key={sub.id} className="grid grid-cols-[1fr_2fr_auto] gap-3 items-center animate-in slide-in-from-top-1 duration-200">
                                <select 
                                    value={sub.language}
                                    onChange={(e) => handleSubtitleChange(sub.id, 'language', e.target.value)}
                                ...
                                >
                                    ...
                                    <option value="Tamil" className="bg-[#1E1E1E]">Tamil</option>
                                    <option value="Hindi" className="bg-[#1E1E1E]">Hindi</option>
                                    <option value="Spanish" className="bg-[#1E1E1E]">Spanish</option>
                                </select>
                                ...
                                <div className="relative group/sub">
                                    <input 
                                        type="file" 
                                        accept=".vtt" 
                                        onChange={(e) => handleSubtitleChange(sub.id, 'file', e.target.files[0])}
                        ...
                        >
                            ...
                            <span className="text-[12px] font-bold text-orange-500/80 group-hover:text-orange-500 transition-colors">Add subtitle</span>
                        </button>
                    </div>
                    ...
                    <div className="p-3 bg-orange-500/[0.03] border border-orange-500/10 rounded-xl flex gap-2.5">
                        ...
                        <p className="text-[10.5px] leading-tight text-orange-500/60 font-medium italic">
                            Videos uploaded here can't be downloaded later. Please keep a backup for your later use.
                        </p>
                    </div>
                    ...
                    >
                        {isSaving ? (
                            ...
                            isEditing ? 'Update Video' : 'Start Video Processing'
                        )}
                    </button>
                </div>
            </div>
