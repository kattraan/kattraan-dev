...
const AnalyticsOverview = () => {
  ...
  const metrics = useMemo(() => {
    ...
    const satisfactionValue =
      statsData?.avgRating != null ? `${statsData.avgRating}/5` : "—";
  ...
  return (
    <DashboardLayout
      title="Analytics"
      subtitle="Deep dive into your course performance."
    >
      <div className="space-y-10">
          ...
          {metrics.map((stat, i) => (
            ...
            >
              <div className="mb-4 flex items-start justify-between">
                ...
                <span className="text-[10px] font-bold text-gray-400 dark:text-white/40 transition-colors duration-300">
                  {loading ? "—" : stat.trend}
                </span>
              </div>
              <p className="text-gray-500 dark:text-white/40 text-[11px] font-bold uppercase tracking-widest leading-none transition-colors duration-300">
                {stat.label}
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="rounded-[32px] border border-gray-200 bg-white/95 p-12 text-center shadow-sm backdrop-blur-sm transition-colors duration-300 dark:border-white/[0.12] dark:bg-white/[0.06] dark:shadow-[0_8px_32px_rgba(0,0,0,0.45)] dark:backdrop-blur-xl">
            <h3 className="mb-2 text-xl font-bold text-gray-900 transition-colors duration-300 dark:text-white">
              Revenue Growth
            </h3>
            <p className="text-gray-500 transition-colors duration-300 dark:text-white/40">
              {loading
                ? "Loading…"
                : `Total revenue: ₹${Math.round(statsData?.totalRevenue || 0).toLocaleString("en-IN")}`}
            </p>
          </div>
          <div className="rounded-[32px] border border-gray-200 bg-white/95 p-12 text-center shadow-sm backdrop-blur-sm transition-colors duration-300 dark:border-white/[0.12] dark:bg-white/[0.06] dark:shadow-[0_8px_32px_rgba(0,0,0,0.45)] dark:backdrop-blur-xl">
            <h3 className="mb-2 text-xl font-bold text-gray-900 transition-colors duration-300 dark:text-white">
            ...
            <p className="text-gray-500 transition-colors duration-300 dark:text-white/40">
              {loading
                ? "Loading…"
                : `${statsData?.totalLearners || 0} active learners across ${statsData?.publishedCourses || 0} live courses`}
            </p>
          </div>
        </div>
      </div>
