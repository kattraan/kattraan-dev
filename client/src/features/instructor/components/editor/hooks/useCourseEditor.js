...
import { updateCourse, submitForReview } from '@/features/courses/store/courseSlice';
...
export function useCourseEditor() {
  ...
  const { data: courseData, isLoading: queryLoading, refetch: refetchCourse } = useGetCourseByIdQuery(id, { skip: !id });
  ...
  const [activeDripType, setActiveDripType] = useState('Learner enrollment date');
  ...
  const [quizPreferredAssessmentMode, setQuizPreferredAssessmentMode] = useState('quiz');
  ...
  const normalizeStatus = (s) => {
    ...
    return ['draft', 'pending_approval', 'published', 'rejected'].includes(v) ? v : 'draft';
  };
  ...
  useEffect(() => {
    if (!id || !courseDetails?.sections?.length || durationBackfillRunRef.current) return;
  ...
  const handleVideoUploadComplete = useCallback(
    async (payload, done) => {
      ...
      try {
        ...
        const { title: t, message: m } = err.apiMessageForToast || { title: 'Save Failed', message: 'Failed to save video lesson' };
        toast.error(t, m);
  ...
  const handleVideoUploadError = useCallback(
    (err, key) => {
      const { title: t, message: m } = err?.apiMessageForToast || { title: 'Upload Failed', message: err?.message || 'Video upload failed' };
      toast.error(t, m);
    },
    [toast]
  );
  ...
  const { uploadState: videoUploadState, startUpload: startVideoUpload, clearUpload: clearVideoUpload, retryUpload: retryVideoUpload } = videoUpload;
  ...
  const handleSave = useCallback(
    async (status = null, shouldLoad = true, showToast = true) => {
      ...
      try {
        ...
        const { title, message } = error.apiMessageForToast || { title: 'Save Failed', message: 'Failed to save course information' };
  ...
  const handleSubmitForReview = useCallback(
    async () => {
      ...
      try {
        ...
        const message = payload.message || err.message || 'Failed to submit for review.';
        const errors = Array.isArray(payload.errors) ? payload.errors : [];
        const detail = errors.length ? errors.join(' ') : message;
        const title = errors.length ? 'Course is not ready for review' : 'Submit failed';
  ...
  const addSection = useCallback(
    async (title) => {
      try {
        ...
        const { title, message } = error.apiMessageForToast || { title: 'Creation Failed', message: 'Failed to create section' };
        toast.error(title, message);
      }
    },
    [id, loadCourse, toast]
  ...
  const deleteSection = useCallback(
    async (sectionId) => {
      const confirmed = await confirm({
        ...
        message: 'This action cannot be undone. All chapters in this section will also be deleted.',
        confirmText: 'Delete',
        variant: 'danger',
      });
      if (confirmed) {
        try {
          ...
          const { title: t, message: m } = error.apiMessageForToast || { title: 'Deletion Failed', message: 'Failed to delete section' };
          toast.error(t, m);
        }
      }
    },
  ...
  const updateSection = useCallback(
    async (sectionId, title) => {
      try {
        ...
        const { title: t2, message: m2 } = error.apiMessageForToast || { title: 'Update Failed', message: 'Failed to update section' };
        toast.error(t2, m2);
      }
    },
    [loadCourse, toast]
  ...
  const addChapter = useCallback(
    async (sectionId, title = 'New Chapter') => {
      try {
        const response = await courseService.createChapter({ section: sectionId, title });
        if (response.success) {
        ...
        const { title: t3, message: m3 } = error.apiMessageForToast || { title: 'Creation Failed', message: 'Failed to create chapter' };
        toast.error(t3, m3);
      }
    },
    [loadCourse, toast]
  ...
  const updateChapter = useCallback(
    async (sectionId, chapterId, updates) => {
    ...
    async (chapterId) => {
      ...
      if (confirmed) {
        try {
          ...
          const { title: t4, message: m4 } = error.apiMessageForToast || { title: 'Deletion Failed', message: 'Failed to delete chapter' };
          toast.error(t4, m4);
        }
      }
    },
  ...
  const deleteContent = useCallback(
    async (type, contentId) => {
      const confirmed = await confirm({
        ...
        message: 'This action cannot be undone and the content will be permanently removed.',
        confirmText: 'Delete',
        variant: 'danger',
      });
      if (confirmed) {
        try {
          ...
          toast.success('Deleted', `${type.charAt(0).toUpperCase() + type.slice(1)} deleted successfully!`);
          await loadCourse();
        } catch (error) {
          logger.error('Failed to delete content', error);
          const { title: t5, message: m5 } = error.apiMessageForToast || { title: 'Deletion Failed', message: 'Failed to delete content' };
          toast.error(t5, m5);
        }
      }
    },
  ...
  const moveSection = useCallback(
    async (fromIdx, toIdx) => {
      ...
      try {
        const sectionIds = newSections.map((sec) => (sec._id || sec.id).toString());
  ...
  const moveChapter = useCallback(
    async (sectionId, fromIdx, toIdx) => {
      ...
      try {
        const chapterIds = newChapters.map((ch) => (ch._id || ch.id).toString());
  ...
  const handleContentTrigger = useCallback(
    (type, chapterId, existingQuiz = null, secName = '', chapName = '') => {
      ...
      if (type === 'quiz' || type === 'assignment') {
        setQuizPreferredAssessmentMode(type === 'assignment' ? 'assignment' : 'quiz');
          ...
          courseDetails.sections?.forEach((sec) => {
            ...
            if (chapter) {
              ...
              if (chapter.contents?.length > 0) {
                finalExistingQuiz = chapter.contents.find((c) => c.type === 'quiz');
              }
            }
          });
        }
        ...
        courseDetails.sections?.forEach((sec) => {
          ...
          if (chapter?.contents?.length > 0) {
            existingArticle = chapter.contents.find((c) => c.type === 'article');
          }
        });
      ...
      } else if (['audio', 'image', 'resource'].includes(type) || type === 'pdf' || type === 'document') {
    ...
    async (articleData) => {
      ...
      try {
        ...
        if (editingArticleContent?._id) {
          await courseService.updateContent('article', editingArticleContent._id, contentData);
        ...
        const { title: t6, message: m6 } = error.apiMessageForToast || { title: 'Error', message: 'Failed to save article' };
        toast.error(t6, m6);
      } finally {
        setIsSaving(false);
      }
    ...
    async (linkData) => {
      ...
      try {
        ...
        if (editingLinkContent?._id) {
          await courseService.updateContent('resource', editingLinkContent._id, contentData);
        ...
        const { title: t7, message: m7 } = error.apiMessageForToast || { title: 'Error', message: 'Failed to save link' };
        toast.error(t7, m7);
      } finally {
        setIsSaving(false);
      }
    ...
    (contentId) => {
      for (const section of courseDetails.sections || []) {
        for (const chapter of section.chapters || []) {
          const found = chapter.contents?.find((c) => (c._id || c.id)?.toString() === String(contentId));
  ...
  const updateVideoDescription = useCallback(
    async (contentId, description) => {
      ...
      try {
        ...
        const { title: t, message: m } = error.apiMessageForToast || { title: 'Error', message: 'Failed to update description' };
        toast.error(t, m);
      } finally {
        setIsSaving(false);
      }
  ...
  const closeResourceUpload = useCallback(() => setResourceUploadContent(null), []);

  const openDescriptionModal = useCallback((content) => {
    if (content?.type === 'video') setDescriptionModalContent(content);
  }, []);

  const closeDescriptionModal = useCallback(() => setDescriptionModalContent(null), []);
  ...
  const handleSaveResources = useCallback(
    async (contentId, files) => {
      ...
      try {
        ...
        const { title: t, message: m } = error.apiMessageForToast || { title: 'Error', message: 'Failed to add resources' };
        toast.error(t, m);
      } finally {
        setIsSaving(false);
      }
  ...
  const handleSaveVideo = useCallback(
    (videoData) => {
      const { videoFile, thumbnailFile, subtitles, title, description, resourceFiles } = videoData;
      ...
      const existingUpload = videoUploadState[pendingKey] || (editingVideoContent?._id && videoUploadState[editingVideoContent._id]);
      if (existingUpload && ['uploading', 'processing'].includes(existingUpload.status)) {
        toast.error('Upload in progress', 'Please wait for the current upload to finish.');
        return;
      }
    ...
    [uploadingChapterId, editingVideoContent, videoUploadState, startVideoUpload, toast]
  );
    ...
    async (chapterId, quizData) => {
      ...
      try {
        const formattedQuestions = quizData.questions.map((q) => ({
          question: q.question,
          type: q.type || 'single',
          options: q.type === 'subjective' ? [] : (q.options || []).map((opt) => (typeof opt === 'string' ? opt : opt.content)),
        ...
        if (currentEditingQuiz?._id) {
          response = await courseService.updateContent('quiz', currentEditingQuiz._id, payload);
        } else {
          response = await courseService.createContent('quiz', payload);
        }

        if (response.success && response.data) setCurrentEditingQuiz(response.data);
        await loadCourse();
        toast.success('Success', currentEditingQuiz ? 'Quiz updated successfully!' : 'Quiz created successfully!');
        return response.data;
      } catch (error) {
        logger.error('Quiz creation error:', error);
        const { title, message } = error.apiMessageForToast || { title: 'Creation Failed', message: 'Failed to create quiz. Please try again.' };
        toast.error(title, message);
        throw error;
      } finally {
        setIsSaving(false);
  ...
  const onFileChange = useCallback(
    async (e) => {
      ...
      try {
        ...
        if (uploadRes.success) {
          ...
          if (activeFileUploadType === 'course-cover' || activeFileUploadType === 'course-thumbnail') {
            const key = activeFileUploadType === 'course-cover' ? 'thumbnail' : 'image';
          ...
          if (activeFileUploadType === 'pdf' || activeFileUploadType === 'document') {
            finalType = 'resource';
          }
          ...
          await loadCourse();
          toast.success('Success', `${activeFileUploadType.charAt(0).toUpperCase() + activeFileUploadType.slice(1)} uploaded and added to curriculum!`);
        }
      } catch (error) {
        logger.error('Upload error:', error);
        const { title: t9, message: m9 } = error.apiMessageForToast || { title: 'Upload Failed', message: 'File upload failed. Please try again.' };
        toast.error(t9, m9);
      } finally {
        setIsSaving(false);
        setUploadingChapterId(null);
    ...
    [activeFileUploadType, courseDetails, uploadingChapterId, id, dispatch, loadCourse, toast]
  );
  ...
  const fileInputAccept = useMemo(() => {
    if (activeFileUploadType === 'video' || activeFileUploadType === 'course-thumbnail') return 'video/*';
    if (activeFileUploadType === 'image') return 'image/*';
    if (activeFileUploadType === 'audio') return 'audio/*,.mp3,.wav,.ogg,.m4a,.aac';
    if (activeFileUploadType === 'pdf') return '.pdf';
    if (activeFileUploadType === 'document') return '.doc,.docx,.ppt,.pptx,.txt,.csv';
    return undefined;
